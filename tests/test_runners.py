...
PI_OUT = open(os.path.join(FIX, "pi_turn_end.jsonl"), encoding="utf-8").read()
CC_OUT = open(os.path.join(FIX, "claude_result.json"), encoding="utf-8").read()
...
class TestRunPi(unittest.TestCase):
    def test_parst_echte_ausgabe(self):
        with patch.object(runners, "_run_process", return_value=(PI_OUT, "", False)):
...
class TestRunClaude(unittest.TestCase):
    def test_parst_echte_ausgabe(self):
        with patch.object(runners, "_run_process", return_value=(CC_OUT, "", False)):
...
class TestOverheadBefund(unittest.TestCase):
    ...
    def test_cc_overhead_deutlich_groesser_als_pi(self):
        ...
        self.assertGreater(cc_ov, pi_ov * 3)  # real gemessen ~9.5x
